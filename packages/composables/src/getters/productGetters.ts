...
import { Product } from '@vue-storefront/sylius-api';
...
export const getProductGallery = (
  ...
  product?.galleryImages.map((image, i) => ({
    small: (Array.isArray(product?.images) && product?.images?.length
      ? product.images[i]
      : ''
    ).replace(/\/media\/image/, ''),
    normal: (Array.isArray(product?.galleryImages) &&
...
export const getProductFiltered = (
  ...
  return products.map((product) => {
    const filteredVariant = product.variants.filter((variant) => {
      const filterVariantOptions = variant.optionValues.filter(
        (optionValue) => {
          const attribute = optionValue.option.id.replace(
            '/api/v2/shop/product-options/',
            ''
          );
      ...
      return (
        filterVariantOptions.length === Object.keys(filters.attributes).length
      );
    });
...
export const getProductAttributes = (
        ...
        product.variants.some(
          (variant) =>
            variant.optionValues
              .map((val) => val.id)
              .includes(optionValue.id) && variant.enabled
        )
      )
      .map((optionValue) => ({
  ...
  if (!Array.isArray(products) && product.selectedVariant) {
    for (const optionCode in attributes) {
      const filteredVariant = product.selectedVariant.optionValues.filter(
        (variant) =>
          variant.option.id === `/api/v2/shop/product-options/${optionCode}`
      );

      if (filteredVariant.length)
        configuration[optionCode] = filteredVariant[0].code;
    }
...
export const getProductOptions = (product: Product): any => {
  ...
  for (const index in product.options) {
    const filteredVariant = product.selectedVariant.optionValues.filter(
      (variant) => variant.option.id === product.options[index].id
    );
    options.push({
...
export const getProductStockForVariant = (variant): number =>
  variant ? variant.onHand - variant.onHold : 0;

export const getProductQuantityLimit = (variant) =>
  variant.tracked ? getProductStockForVariant(variant) : 999;

export const isVariantInStock = (variant): boolean =>
  getProductStockForVariant(variant) > 0;

export const productHasMultipleVariants = (product: Product): boolean =>
...
// eslint-disable-next-line @typescript-eslint/no-unused-vars
export const getProductAverageRating = (product: Product): number =>
