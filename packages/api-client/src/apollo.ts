...
export const handleRetry = () => (count, operation, error) => {
  ...
  if (error?.result?.message === 'invalid_token') {
    ...
    Logger.debug(
      `Apollo retry-link, the operation (${operation.operationName}) sent with wrong token, creating a new one... (attempt: ${count})`
    );
    return true;
  }
...
export const authLinkFactory = ({ state, customHeaders }) =>
  setContext((apolloReq, { headers }) => {
